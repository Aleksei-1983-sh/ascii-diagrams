...
void
conn_set_control_point(int idx, int wx, int wy)
{
	if (idx < 0 || idx >= conn_count_v)
		return;
...
void
conn_clear_control_point(int idx)
{
	if (idx < 0 || idx >= conn_count_v)
		return;
...
draw_straight_horizontal_world(int wx_left_border, int wx_right_border, int wy, int direction)
{
	...
	int end = right - 1;
	if (start > end)
		return; /* нет места между рамками */
...
/* Нарисовать вертикальный сегмент world (wx fixed) от top_border до bottom_border (exclusive
   endpoints) direction: +1 top->bottom, -1 bottom->top.
*/
static void
...
static void
draw_straight(int x0, int y0, int x1, int y1)
{
	if (x0 == x1)
	{
		if (y0 <= y1)
		{
			...
			{
				int sx, sy;
				world_to_screen(x0, y, &sx, &sy);
				put_screen_char(sx, sy, '|');
			}
			int sx, sy;
			world_to_screen(x1, y1 - 1, &sx, &sy);
			put_screen_char(sx, sy, 'v');
		}
		else
		{
			for (int y = y0; y > y1; --y)
	...
	else if (y0 == y1)
	{
		if (x0 <= x1)
		{
			...
			{
				int sx, sy;
				world_to_screen(x, y0, &sx, &sy);
				put_screen_char(sx, sy, '-');
			}
			int sx, sy;
			world_to_screen(x1 - 1, y1, &sx, &sy);
			put_screen_char(sx, sy, '>');
		}
		else
		{
	...
	else
	{
		...
		int sx, sy;
		world_to_screen(x0, y0, &sx, &sy);
		put_screen_char(sx, sy, '*');
		world_to_screen(x1, y1, &sx, &sy);
		put_screen_char(sx, sy, '*');
	}
}
...
draw_orthogonal_L_from_points(int ax, int ay, int corner_x, int corner_y, int bx, int by,
			      int final_is_horizontal, conn_t *conn)
{
	LOG_CONN("a=(%d,%d) d=(%d,%d) conrner=(%d,%d) final_is_horizontal=%d ida=%d idb=%d", ax, ay,
		 bx, by, corner_x, corner_y, final_is_horizontal, conn->a, conn->b);
	/* вертикальная часть (ax==corner_x предпочтительно) */
	if (ax == corner_x)
	{
		if (ay <= corner_y)
		{
			for (int y = ay; y < corner_y; ++y)
			{
				int sx, sy;
				world_to_screen(ax, y, &sx, &sy);
				put_screen_char(sx, sy, '|');
			}
		}
		else
	...
	{
		int sx, sy;
		world_to_screen(corner_x, corner_y, &sx, &sy);
		put_screen_char(sx, sy, '+');
	}
	...
	if (conn != NULL)
	{
		if (final_is_horizontal)
		{
			...
			{
				draw_straight_horizontal_world(rec_b->x + rec_b->w - 1, corner_x,
							       corner_y, -1);
			}
			else
			{
...
*/
static void
draw_conn_world(conn_t *conn, Rect *ra, Rect *rb)
{
	if (!conn || !ra || !rb)
		return;
	...
	LOG_CONN("a=(%d,%d) b=(%d,%d) has_control=%d control=(%d,%d)", ax, ay, bx, by,
		 conn->has_control, conn->point_control.x, conn->point_control.y);
	...
	if (conn->has_control)
	{
		...
		/* для каждой части решаем ориентацию финального сегмента, основываясь на стороне
		   цели:
		   - для части A->control: цель = control, у нас нет rect для control, используем
		   эвристику: если cx == ax -> финал горизонтален; если cy == ay -> финал
		   вертикален; иначе по abs.
		   - для части control->B: цель = B (rb) — можем определить сторону B по bx,by и rb.
		*/
		/* часть A->control: угол будет (ax, cy) */
		int final_h_A =
		    (cx == ax) ? 1 : ((cy == ay) ? 0 : (abs(cx - ax) >= abs(cy - ay) ? 1 : 0));
		...
		if (bx == rb->x || bx == rb->x + rb->w - 1)
			final_h_B = 1;
		else if (by == rb->y || by == rb->y + rb->h - 1)
			final_h_B = 0;
		else
			final_h_B = (abs(bx - cx) >= abs(by - cy)) ? 1 : 0;

		draw_orthogonal_L_from_points(cx, cy, cx, by, bx, by, final_h_B, NULL);
		return;
	}

	/* Без контрольной точки: выбираем ориентацию финального сегмента по тому, на какой стороне
	 * цели (rb) лежит точка B. */
	int final_is_horizontal = 1;
	if (bx == rb->x || bx == rb->x + rb->w - 1)
		final_is_horizontal = 1;
	else if (by == rb->y || by == rb->y + rb->h - 1)
		final_is_horizontal = 0;
	else
		final_is_horizontal = (abs(bx - ax) >= abs(by - ay)) ? 1 : 0;
...
conn_draw_all(void)
{
	...
	for (int i = 0; i < conn_count_v; ++i)
	{
		...
		for (int j = 0; j < rect_count(); ++j)
		{
			...
			if (r->id == conns[i].b)
				index_rect_b = j;
			if (index_rect_a != -1 && index_rect_b != -1)
				break;
		}

		if (index_rect_a < 0 || index_rect_b < 0)
		{
			LOG_CONN("conn[%d] skip: missing rects a=%d b=%d", i, conns[i].a,
				 conns[i].b);
			continue;
		}
		...
		{
			LOG_CONN("conn[%d] skip: rectangles overlap a=%d b=%d", i, conns[i].a,
				 conns[i].b);
			continue;
		}
		...
		/* Попытки прямого соединения (горизонтального/вертикального) на основе реального
		 * расположения блоков */
		...
		if (ra->x + ra->w - 1 < rb->x)
		{
			...
			if (inter_top <= inter_bottom)
			{
				...
				{
					LOG_CONN("conn[%d] straight horizontal chosen (wy=%d) from "
						 "%d to %d",
						 i, wy, a_border_x, b_border_x);
					draw_straight_horizontal_world(a_border_x, b_border_x, wy,
								       +1);
					continue;
				}
			}
		...
		if (rb->x + rb->w - 1 < ra->x)
		{
			...
			if (inter_top <= inter_bottom)
			{
				...
				{
					LOG_CONN("conn[%d] straight horizontal mirrored chosen "
						 "(wy=%d) from %d to %d",
						 i, wy, b_border_x, a_border_x);
					draw_straight_horizontal_world(b_border_x, a_border_x, wy,
								       -1);
					continue;
				}
			}
		...
		if (ra->y + ra->h - 1 < rb->y)
		{
			...
			if (inter_left <= inter_right)
			{
				...
				{
					LOG_CONN("conn[%d] straight vertical chosen (wx=%d) from "
						 "%d to %d",
						 i, wx, a_border_y, b_border_y);
					draw_straight_vertical_world(wx, a_border_y, b_border_y,
								     +1);
					continue;
				}
			}
		...
		if (rb->y + rb->h - 1 < ra->y)
		{
			...
			if (inter_left <= inter_right)
			{
				...
				{
					LOG_CONN("conn[%d] straight vertical mirrored chosen "
						 "(wx=%d) from %d to %d",
						 i, wx, b_border_y, a_border_y);
					draw_straight_vertical_world(wx, b_border_y, a_border_y,
								     -1);
					continue;
				}
			}
...
*/
void
conn_draw_temporary_to_mouse(int conn_idx, int mx, int my)
{
	if (conn_idx < 0 || conn_idx >= conn_count_v)
		return;
	...
	for (int j = 0; j < rect_count(); ++j)
	{
		if (rect_get(j)->id == conns[conn_idx].a)
		{
			ia = j;
			break;
		}
...
conn_hit_at(int mx, int my)
{
	for (int i = 0; i < conn_count_v; ++i)
	{
		...
		if (conns[i].has_control)
		{
			 ...
			 */
			if (mx == ax && between_i(my, ay, cy))
				return i;
			if (my == cy && between_i(mx, ax, cx))
				return i;
			/* control -> B: vertical (cx,cy)->(cx,by) and horizontal (cx,by)->(bx,by)
			 */
			if (mx == cx && between_i(my, cy, by))
				return i;
			if (my == by && between_i(mx, cx, bx))
				return i;
		}
		else
		{
