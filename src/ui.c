...
void draw_button(void)
{
    ...
    mvaddstr(BTN_Y, BTN_X + (int)strlen(BTN_TEXT) + 2, "(left click)");
}
...
void ui_draw_all(int editing, int edit_idx, int conn_move_active, int conn_selected, int last_mouse_x, int last_mouse_y)
{
    ...
    conn_draw_all(); /* предполагается, что conn_draw_all теперь работает с rect.get_border_point (world coords) и сам делает перевод в screen */
    ...
    if (editing && edit_idx >= 0) panel_draw(rect_get(edit_idx));
