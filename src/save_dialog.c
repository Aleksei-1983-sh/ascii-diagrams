 ...
 * Implements the save/load dialog UI logic: browsing directories, selecting files,
 * and confirming where diagram data should be stored or loaded from.
 */
...
#define DLG_W 70
#define DLG_MAX_LIST_H 10

typedef struct
{
char name[DLG_NAME_MAX];
int is_dir;
int is_diag; /* 1 если файл .diag */
} DialogItem_t;
...
return -1;
if (s->background == NULL && s->mode == 0)
return -1;

if (s->mode == 0)
return storage_save_visual(path, s->background, s->bg_w, s->bg_h);
else
return storage_load_text(path);
}
...
title = s->mode == 0 ? " Save diagram " : " Load diagram ";
btn_label = s->mode == 0 ? "[Save]   [Cancel]" : "[Load]   [Cancel]";

ui_draw_box(x0, y0, DLG_W, dlg_h, title);
...
marker = " *";
snprintf(line, sizeof(line), "%c %s%s", idx == s->selected ? '>' : ' ', it->name, marker);
}
else
{
snprintf(line, sizeof(line), " ");
}
mvprintw(y0 + 3 + i, x0 + 2, "%-65s", line);
}

mvprintw(y0 + 4 + list_h, x0 + 2, "File name: %-54s", s->file_name);
mvaddstr(y0 + 5 + list_h, x0 + 2, btn_label);
mvaddstr(y0 + 5 + list_h, x0 + 22, "   (mouse: click items/buttons)");
refresh();
}
