...
static RectNode *
node_at_index(int idx)
{
	if (idx < 0 || idx >= rect_count_v)
		return NULL;
	RectNode *cur = head;
	int i = 0;
	while (cur && i < idx)
...
int
rect_hit_resize_handle(Rect *r, int mx, int my)
{
...
rect_clamp(Rect *r)
{
	...
	if (r->y < WORLD_MIN_Y)
		r->y = WORLD_MIN_Y;
	if (r->x + r->w > WORLD_MAX_X)
		r->x = WORLD_MAX_X - r->w;
	if (r->y + r->h > WORLD_MAX_Y)
		r->y = WORLD_MAX_Y - r->h;
}

/* Текстовые: разбиение на строки одинаково — остаётся прежним */
int
rect_wrap_text(const char *text, int inner_w, char out_lines[][256], int max_lines)
{
 ...
 */
void
rect_get_border_point(Rect *r, int tx, int ty, int *outx, int *outy)
{
	...
	LOG_RECT("rect id=%d center=(%.3f,%.3f) target=(%d,%d) dir=(%.3f,%.3f)", r->id, centerX,
		 centerY, tx, ty, dirX, dirY);
	...
	/* Пределы допустимой области пересечения (с допуском 0.5 для корректного попадания в
	   символную сетку). Мы проверяем попадание пересечения в отрезок стороны: вертикальные
	   стороны: y ∈ [r->y - 0.5, r->y + r->h - 0.5] горизонтальные стороны: x ∈ [r->x - 0.5,
	   r->x + r->w - 0.5]
	*/
	...
	if (bestT < INFINITY)
	{
		...
		if (resultX < r->x)
			resultX = r->x;
		if (resultX > r->x + r->w - 1)
			resultX = r->x + r->w - 1;
		if (resultY < r->y)
			resultY = r->y;
		if (resultY > r->y + r->h - 1)
			resultY = r->y + r->h - 1;

		*outx = resultX;
		*outy = resultY;

		LOG_RECT("chosen border point t=%.6f -> raw=(%.3f,%.3f) -> rounded=(%d,%d)", bestT,
			 bestX, bestY, *outx, *outy);
		return;
	}
...
auto_safe:
	...
	if (sx + w - 1 >= 0 && sx + w - 1 < COLS)
		mvaddch(sy, sx + w - 1, '*');
	if (sy + h - 1 >= 0 && sy + h - 1 < LINES)
		mvaddch(sy + h - 1, sx, '*');
	if (sx + w - 1 >= 0 && sx + w - 1 < COLS && sy + h - 1 >= 0 && sy + h - 1 < LINES)
		mvaddch(sy + h - 1, sx + w - 1, '*');
	...
	if (sy >= 0 && sy < LINES)
	{
		...
		if (cx < 0)
		{
			...
			if (skip < lablen)
			{
				mvaddnstr(sy, 0, label + skip,
					  (lablen - skip) < maxlen ? (lablen - skip) : maxlen);
			}
		}
		else
		{
...
void
rect_draw_text_centered(Rect *r)
{
	if (!r)
		return;
	int inner_w = r->w - 2;
